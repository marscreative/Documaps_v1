        ...
        private void dataGridView1_CellContentClick(object sender, DataGridViewCellEventArgs e)
        {
            try
            {
                ...
                else if (colname == "btnDelete")
                {
                    ...
                    {
                        string residentId = dataGridView1.Rows[e.RowIndex].Cells[0].Value.ToString();

                        cn.Open();

                        // Delete from tblvaccine
                        cm = new SqlCommand("DELETE FROM tblvaccine WHERE rid = @residentId", cn);
                        cm.Parameters.AddWithValue("@residentId", residentId);
                        cm.ExecuteNonQuery();

                        // Delete from tblresident
                        cm = new SqlCommand("DELETE FROM tblresident WHERE id = @residentId", cn);
                        cm.Parameters.AddWithValue("@residentId", residentId);
                        cm.ExecuteNonQuery();
